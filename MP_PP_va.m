...
x0 = [0,0,deg2rad(0),deg2rad(0),0,-2000]';
%goal = [300,600,0,0,0,-2400];
goal = [330,120,0,0,0,-2100];  %depth = 3, eps = 100;
...
mynodes.S = [0];
mynodes.T = [1];
mynodes = addNode(mynodes,x0(1),x0(2),x0(3),x0(4),x0(5),x0(6));
previous_nodes = [x0];
new_nodes = [];
mynodes.mp = [0];
...
while num < depth
    ...
    for kk = 1:size(previous_nodes,2)
        ...
        for i = 1:n
            ...
            wind_effect = Ts * [wind_inertial(1),wind_inertial(2),0,0,0,wind_inertial(3)]';
            ...
            if flag_x1_thm1
                ...
                wind_effect = [wind_effect(1),wind_effect(2),0,0,0,wind_effect(3)]';
                x1 = x_old + x_del' + wind_effect;
                pt1 = [x1(1),x1(2),x1(end)]';
            ...
            mynodes = addNode(mynodes,x1(1),x1(2),x1(3),x1(4),x1(5),x1(6));
...
for i = 1:length(smp_path)
    ...
    for j = 1:length(stops{i})-1
        ...
        if MP_INDEX < length(str_MP) + 1
            ...
            Res=AirRelativeVelocityToWindAngles(as(7:9));
            x_r = [as(1),as(2),as(6),U(3),Res(1),as(3)]';
            head = as(6);
        ...
        % Splined curves
            ...
            Res=AirRelativeVelocityToWindAngles(as(7:9));
            x_r = [as(1),as(2),as(6),a(3),Res(1),as(3)]';
            head = as(6);
            ...
            Res=AirRelativeVelocityToWindAngles(as(7:9));
            x_r = [as(1),as(2),as(6),b(3),Res(1),as(3)]';
            head = as(6);
        end
    end
    full_dyn_pts{i} = PTS;
...
col = 'r';
PlotSimulation(time, as_all_array{1},acs_all_array{1},col,goal)
figure;
fntsz = 14;
xlabel("x [m]",'fontsize',fntsz);ylabel("y [m]",'fontsize',fntsz);zlabel("z [m]",'fontsize',fntsz);
grid on;
...
title("3D Comparision between full-dyn and simple-dyn",'fontsize',fntsz)
for i = 1:l
...
title("2D Comparision between full-dyn and simple-dyn",'fontsize',fntsz)
for i = 1:l
    plot3(smp_path{i}(1,:)',smp_path{i}(2,:)',-smp_path{i}(end,:)','b')
    plot3(stops_red{i}(1,:)',stops_red{i}(2,:)',-stops_red{i}(end,:)','ro','markersize',8,'markerfacecolor','r')
end
