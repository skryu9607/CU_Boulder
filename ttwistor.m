...
aircraft_parameters.kmotor = 30; %25;%40
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  aircraft_parameters.Cmde =  -0.06; %-0.028684; EWF Changed 5/8/15 from AVL values to give better performance
  %aircraft_parameters.CDde =   0.000059; derived by AVL, not used
  %aircraft_parameters.ede =   0.014857;

  % Aileron
  aircraft_parameters.CYda =  -0.000754;
  aircraft_parameters.Clda =  -0.02; %-0.006290; EWF Changed 5/8/15 from AVL values to give better performance
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Alternative linear formulation for drag terms 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  
%Trim flight condition values 
aircraft_parameters.height = 1600; % Altitude [m]
aircraft_parameters.rho = stdatmo(aircraft_parameters.height);%0.963;    % Density [kg/m^3]
aircraft_parameters.u0 = 21;   %[m/s] true air speed
...
aircraft_parameters.CW0 = 0.3835;   % Not used in new drag model, but kept in for backwards compatibility just in case.
  
  % Trim state and control variables
aircraft_parameters.theta0 = 0.0265; %[rad]
aircraft_parameters.alpha0 = aircraft_parameters.theta0; %[rad], angle of attack

aircraft_parameters.de0 = .1429;  % [rad], elevator deflection at trim
aircraft_parameters.dt0 = 0.3053; % [], throttle setting at trim

aircraft_parameters.CD0 = aircraft_parameters.CDmin + aircraft_parameters.K*aircraft_parameters.CW0*aircraft_parameters.CW0;
aircraft_parameters.CDalpha = 2*aircraft_parameters.K*aircraft_parameters.CW0*aircraft_parameters.CLalpha; %[-], Calculated from drag polar, general sense consistent with Roskam if CLalpha positive
aircraft_parameters.CDq = 2*aircraft_parameters.K*aircraft_parameters.CW0*aircraft_parameters.CLq; %[-], Calculated from drag polar, general sense consistent with Roskam if CLalpha positive
aircraft_parameters.CDde = 2*aircraft_parameters.K*aircraft_parameters.CW0*aircraft_parameters.CLde;

aircraft_parameters.gamma = Inertial_gamma(aircraft_parameters.Ix,aircraft_parameters.Iy,aircraft_parameters.Iz,aircraft_parameters.Ixz);
